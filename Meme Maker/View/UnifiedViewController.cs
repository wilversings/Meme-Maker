...
namespace MemeMaker {
    static class UnifiedViewController {
        ...
        public static void HandleDragEnter (object sender, DragEventArgs e, MemeService memeService) {
            string[] droppedFiles;
            try {
        ...
        public static void HandleDragDrop (object sender, DragEventArgs e, MemeService memeService) {

            string[] droppedFiles;
            // We passed the DragEnter event, and we can assure that the user dropped
            // files, with the right format
            droppedFiles = e.Data.GetData (DataFormats.FileDrop) as string[];
            droppedFiles = droppedFiles.Where (f => MemeService.IsAcceptedFileFormat (f) && !memeService.HasLoadedPath (f))
                                       .ToArray ();
